...
{
	if(ph->numOfData ==0) // 자식 노드가 없다면 TRUE 
		return TRUE;
	else
		return FALSE;
}
...
int GetHiPriChildIDX(Heap * ph, int idx)
{
	if(GetLChildIDX(idx) > ph->numOfData) // 자식 노드가 존재하지 않는다면
		return 0; //
	else if(GetLChildIDX(idx) == ph->numOfData) // 자식 노드가 왼쪽 자식 노드 하나만 존재한다면 7
...
HData HDelete(Heap * ph)
{
	...
	while(childIdx = GetHiPriChildIDX(ph, parentIdx))
	{
		...
		// 마지막 노드보다 childIdx의 우선순위가 더 높으니, 비교대상 노드의 위치를 한 레벨 올림 
